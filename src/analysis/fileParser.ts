...
export function detectFileType(filePath: string): "csv" | "json" | null {
...
export function parseFile(filePath: string, sampleSize?: number): ParseResult {
  const fileType = detectFileType(filePath);
  if (!fileType) throw new Error(`Unsupported file type: ${path.extname(filePath)}`);
  if (fileType === "csv") return parseCsv(filePath, sampleSize);
  return parseJson(filePath, sampleSize);
}

export function buildMetadata(filePath: string, result: ParseResult): FileMetadata {
...
function readFileWithEncoding(filePath: string): { content: string; encoding: string } {
  ...
  if (buffer.length === 0) return { content: "", encoding: "utf-8" };
...
function parseCsv(filePath: string, sampleSize?: number): ParseResult {
  const { content, encoding } = readFileWithEncoding(filePath);
  if (content.trim().length === 0) {
    return { headers: [], rows: [], totalRowCount: 0, sampled: false, encodingUsed: encoding, flatteningApplied: false };
  }
  ...
  return { headers, rows, totalRowCount, sampled, encodingUsed: encoding, flatteningApplied: false };
}

function parseJson(filePath: string, sampleSize?: number): ParseResult {
  const { content, encoding } = readFileWithEncoding(filePath);
  if (content.trim().length === 0) {
    return { headers: [], rows: [], totalRowCount: 0, sampled: false, encodingUsed: encoding, flatteningApplied: false };
  }
  ...
  return { headers, rows, totalRowCount, sampled, encodingUsed: encoding, flatteningApplied };
}
